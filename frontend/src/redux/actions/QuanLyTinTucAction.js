...
export const layKetQuaTimKiem = (content) => {
    ...
    return async (dispatch) => {
        try {
            // Gọi API search phim đúng
            const result = await quanLyPhimService.searchPhim(content);
            dispatch({
                type: LAY_KET_QUA_TIM_KIEM,
                arrTimKiem: { movie: result.data.content, news: [] }, // news để trống nếu chưa có API search news
            })
        } catch (error) {
            console.log('error', error);
        }
